...
pose_track.py
─────────────
* Tracks a person with MediaPipe Pose.
* Sends B1:<dir><offset>,<dir><offset> via pi_bt.ble_advertiser.
* Reads servo feedback A1:<pan>,<tilt> from your existing
  pi_bt.ble_scanner.BLEScannerThread – but **uses it only while sweeping**,
  so stale packets don’t disturb active tracking.
"""
...
# ────────────────────────────────────────────
# add your project directory and import helpers
# ────────────────────────────────────────────
...
KP, KI, KD            = 2.8, 0.02, 0.4   # PID gains (pixel domain)
DEAD_PIX, MAX_STEP_US = 4, 80

SWEEP_SPAN, SWEEP_STEP = 120, 20         # ±span°, step° during sweep
MISS_TOL, ACQ_FRAMES   = 8, 4            # frames lost / acquired

BLE_DELTA_MIN, BLE_HOLD_MS = 10, 150     # µs change / min TX spacing
...
class PID:
    ...
    def prime(self, start): self.prev_out = start
    def update(self, err):
...
def main():
    ...
    scanner.start()                     # scanner runs continuously

    pid_pan = PID(KP,KI,KD, DEAD_PIX, MAX_STEP_US)
    pid_tlt = PID(KP,KI,KD, DEAD_PIX, MAX_STEP_US)
    sweep   = Sweep(SWEEP_SPAN, SWEEP_STEP)

    state = "SWEEP"; miss = acq = 0
    fb_pan = fb_tilt = 0                # last feedback from A1:
    last_tx = 0; last_p = last_t = 0
    ...
    with mp_p.Pose(model_complexity=0,min_detection_confidence=.5,min_tracking_confidence=.5) as pose:
        try:
            while True:
                # ---- read BLE queue (always running, use only in SWEEP) ---
                try:
                    while True:
                ...
                if res.pose_landmarks:
                    ...
                    x1,y1,x2,y2=body_box(lm,w,h,60); cx,cy=(x1+x2)//2,(y1+y2)//2
                ...
                # ---- transmit (rate-limited) ------------------------------
                now=time.time()
                if abs(pan_us-last_p)>=BLE_DELTA_MIN or abs(tilt_us-last_t)>=BLE_DELTA_MIN \
                   or (now-last_tx)*1000>BLE_HOLD_MS:
