...
def main():
    last_distance = None
    last_gesture = None
    distance = None  # temporary holder for new ultrasonic reading
        ...
        try:
            while True:
                ...
                if res.pose_landmarks:
                    ...
                    pan_val = (1 if pan >= 0 else 0) * 1000 + min(abs(int(pan)), 999)
                    tilt_val = (1 if tilt >= 0 else 0) * 1000 + min(abs(int(tilt)), 999)

                    if pan_val != advertiser.pan:
                        advertiser.update_pan(pan_val)

                    if tilt_val != advertiser.tilt:
                        advertiser.update_tilt(tilt_val)
                else:
                    if not lost_active and time.time() - last_seen > LOST_TIMEOUT:
                        lost_active = True
                        pid_pan.reset()
                        pid_tilt.reset()
                        if SPECIAL_CMD != advertiser.pan:
                            advertiser.update_pan(SPECIAL_CMD)
                        if SPECIAL_CMD != advertiser.tilt:
                ...
                if gesture != advertiser.gesture:
                    advertiser.update_gesture(gesture)

                if gesture != last_gesture or distance != last_distance:
                    payload = {
                        "gesture": gesture,
                        "distance": distance
                    }
                ...
                try:
                    ...
                    if name == "MOBILE" and value.startswith("M1:"):
                        ...
                        new_distance = extract_ultrasonic(value)
                        if new_distance is not None:
                            int_part = int(new_distance)
                            frac_part = int(round((new_distance - int_part) * 100))
                            advertiser.update_integer(int_part)
                            advertiser.update_frac(frac_part)
                            distance = round(new_distance, 2)
                        print(f"[SCAN][ULTRASONIC] DISTANCE: {new_distance} m \n")
